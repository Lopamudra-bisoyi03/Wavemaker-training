document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleCheckboxChange = (taskDiv) => {
    ...
    const saveTasksToLocalStorage = () => {
        ...
        const completedTasks = Array.from(document.querySelector('#completed-tasks').querySelectorAll('.task')).map(task => ({
            text: task.querySelector('.text').value,
            priority: task.dataset.priority,
            due: task.dataset.due,
            completed: true
    ...
    const loadTasksFromServer = async () => {
        try {
            ...
            console.log("http://localhost:8082/To-Do-App/todos");
            const response = await fetch('http://localhost:8082/To-Do-App/todos');
            if (!response.ok) {
            ...
            // Process each task
            tasks.forEach(task => {
                addTask(task.text, task.priority, task.due);

                // Get the most recently added task element
                const taskDiv = tasksContainer.querySelector('.task:last-child');

                // Update the task's checkbox state and move to the correct section
                const checkbox = taskDiv.querySelector('.complete-checkbox');
                checkbox.checked = task.completed;

                // Move the task to the completed section if it is marked as completed
                if (task.completed) {
                    document.querySelector('#completed-tasks').appendChild(taskDiv);
                }
            });
...
// Call this function to load tasks when the page loads
    loadTasksFromServer();
   ...
   // loadTasksFromLocalStorage();
    ...
    // Request notification permission and show notifications if granted
    Notification.requestPermission().then(permission => {
        if (permission === 'granted') {
